...
test('Safe proposal digest binds chain, wallet, destination, value, calldata and nonce', () => {
  ...
  for (const changed of [
    ...
    { ...transaction, value: '2' }, { ...transaction, data: '0x1234' as const }, { ...transaction, nonce: '1' },
...
test('Safe input decoding rejects malformed integers and hidden authority fields', () => {
  ...
  expect(() => decode({ ...input, transaction: { ...input.transaction, operation: 1 } }, { onExcessProperty: 'error' })).toThrow()
