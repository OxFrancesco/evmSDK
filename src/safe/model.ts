...
export const SafeUint = Uint.check(Schema.isMaxLength(78), Schema.makeFilter(value => (/^(0|[1-9][0-9]*)$/.test(value) && BigInt(value) < 2n ** 256n) || 'Value exceeds uint256'))
...
export const SafeInfo = Schema.Struct({ ...SafeTarget.fields, ...SafeOwners.fields, version: Schema.Literal('1.4.1'), modules: Schema.Array(Address), nonce: SafeUint, block: Uint })
export interface SafeInfo extends Schema.Schema.Type<typeof SafeInfo> {}
export const SafeProposalInput = Schema.Struct({ ...SafeTarget.fields, to: Address, value: SafeUint, data: Hex, operation: Schema.optionalKey(Schema.Literals([0, 1])) })
...
export const SafeOwnerChangeInput = Schema.Struct({
  ...
  change: Schema.Union([
    ...
    Schema.Struct({ kind: Schema.Literal('replace'), owner: Address, replacement: Address }),
    Schema.Struct({ kind: Schema.Literal('threshold'), threshold: SafeOwners.fields.threshold }),
  ]),
})

export const SafeCall = Schema.Struct({ to: Address, value: SafeUint, data: Hex })
export const SafeBatchInput = Schema.Struct({ ...SafeTarget.fields, calls: Schema.Array(SafeCall).check(Schema.isMinLength(1), Schema.isMaxLength(64)) })
